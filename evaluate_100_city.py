...
from nearest_insertion import solve_tsp_nearest_insertion


def evaluate_all_100_city_models(num_trials=100):
    """
    Runs a multi-trial evaluation to compare the average performance of all five models
    on 100-city problems.
    """
    print(f"--- Starting Robust 5-Way Evaluation over {num_trials} Trials (100 Cities) ---")

    # --- Setup ---
    NUM_CITIES = 100
    PURE_RL_PATH = "pure_rl_model_100.pth"
    HYBRID_REINFORCE_PATH = "best_hybrid_reinforce_model_100.pth"
    HYBRID_AC_PATH = "best_ac_actor_model_100.pth"
    ...
    results = {
        ...
        'Nearest Insertion': {'lengths': [], 'times': []},
    ...
    for _ in tqdm(range(num_trials), desc="Running 100-City Evaluation Trials"):
...
if __name__ == '__main__':
    # A smaller number of trials for the larger problem to keep runtime reasonable
    evaluate_all_100_city_models(num_trials=1000)
